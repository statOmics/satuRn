...
`satuRn` (version 1.1.1) can be installed from Bioconductor with:
...
```{r, eval=FALSE}
devtools::install_github("statOmics/satuRn", ref="RELEASE_3_13")
```
...
*! for this vignette, we use the development version of satuRn.*
The vignette for the officially released satuRn version 1.1.1 can be viewed from 
[Bioconductor](https://bioconductor.org/packages/release/bioc/vignettes/satuRn/inst/doc/Vignette.html)
...
the help file of the `testDTU` function. Note that the arguments `diagplot1` and
`diagplot2` were not yet implemented in `satuRn` v1.1.1.
